...
設計重點：
    ...
    - 合成延遲以「每任務釋放偏移（release offset）」建模：跑 CPM 前，
      暫時將該任務的 es 下限抬高（透過注入一個虛擬前置的 ef）。
      具體作法是修改該任務的 duration/predecessors 之外，採後處理夾擠 es，
      故改以重算方式：對每個任務的 es 取 max(CPM es, offset)，
      並沿後繼傳播。為求正確且簡潔，這裡以「加總前置 + 個別下限」重算。
"""
...
from app.core.cpm_engine import _build_graph, _topological_order
...
def _cpm_with_offsets(
    ...
    （前置最大 ef）與 offset 兩者取大值。此即合成延遲的建模方式，
    可在不變更相依結構的前提下，將非要徑任務往後挪移。

    回傳 {task_id: TaskResult}，欄位語意與 cpm_engine.calculate_cpm 相同。
    """
    if not tasks:
        return {}
    ...
    # ---- 前向掃描：es = max(前置 ef, 該任務 offset) ----
    es: dict[str, int] = {}
    ef: dict[str, int] = {}
    for tid in order:
        preds = predecessors[tid]
        base = max((ef[p] for p in preds), default=0)
        es[tid] = max(base, offsets.get(tid, 0))
...
def _build_timeline(
    ...
    for day in range(total_duration):
        ...
        over = any(amount > limits.get(rtype, 0) for rtype, amount in loads.items())
    ...
    for day in range(total_duration):
        ...
        for tid, res in results.items():
            if res.es <= day < res.ef:
                ...
                for rtype, amount in need.items():
                    loads[rtype] += amount
        if any(amount > limits.get(rtype, 0) for rtype, amount in loads.items()):
            return day
    return None
...
def level_resources(
    ...
    if demands and limits:
        ...
        for _ in range(_MAX_ITERATIONS):
            day = _first_over_day(results, demands, limits)
            # 跳過已知無法化解的日子，往後找下一個可處理的超載日。
            while day is not None and day in skip_days:
                day = _next_over_day_after(results, demands, limits, day)
            if day is None:
                break
            ...
            if tid is None:
                ...
                skip_days.add(day)
                _record_unresolved(results, demands, limits, day, unresolved_set)
                continue
    ...
    timeline, over_capacity_days = _build_timeline(results, demands, limits)
